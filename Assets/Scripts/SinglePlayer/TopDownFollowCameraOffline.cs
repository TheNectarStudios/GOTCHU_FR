...
public class TopDownFollowCameraOffline : MonoBehaviour
{
    ...
    private Transform target; // Player target with "Player" tag
    ...
    void LateUpdate()
    {
        // Check if target is still null (in case player was created after Start)
        if (target == null)
        {
            GameObject player = GameObject.FindGameObjectWithTag("Player");
            if (player != null)
